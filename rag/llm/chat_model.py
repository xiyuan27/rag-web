...
class Base(ABC):
    ...
    def _exceptions(self, e, attempt):
        logging.exception("OpenAI cat_with_tools! model_name =" + self.model_name)
        # Classify the error
            ...
            for _ in range(self.max_rounds * 2):
                ...
                try:
                    ...
                    for resp in response:
                        ...
                        finish_reason = resp.choices[0].finish_reason if hasattr(resp.choices[0],
                                                                                 "finish_reason") else ""
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        try:
            ...
            yield ans + "\n**ERROR**: model_name=" + self.model_name + "," + str(e)
...
class QWenChat(Base):
    def __init__(self, key, model_name=Generation.Models.qwen_turbo, base_url=None, **kwargs):
        ...
        self.model_name = model_name
        if self.is_reasoning_model(self.model_name) or self.model_name in ["qwen-vl-plus", "qwen-vl-plus-latest",
    ...
    def chat_with_tools(self, system: str, history: list, gen_conf: dict) -> tuple[str, int]:
        ...
        if not stream_flag:
            ...
            response = Generation.call(self.model_name, messages=history, result_format="message", tools=tools,
                                       **gen_conf)
            ans = ""
            tk_count = 0
            if response.status_code == HTTPStatus.OK:
                ...
                while "tool_calls" in assistant_output:
                    ...
                    response = Generation.call(self.model_name, messages=history, result_format="message",
                                               tools=self.tools, **gen_conf)
                    if response.output.choices[0].get("finish_reason", "") == "length":
                        tk_count += self.total_token_count(response)
                        if is_chinese([ans]):
                ...
                return "**ERROR**: model_name=" + self.model_name + "," + response.message, tk_count
        else:
            result_list = []
            ...
            if len(error_msg_list) > 0:
                return f"**ERROR**: model_name=" + self.model_name + "," + "".join(error_msg_list), 0
            else:
    ...
    def _chat_streamly_with_tools(self, system: str, history: list, gen_conf: dict, incremental_output=True):
        ...
        tk_count = 0
        try:
            response = Generation.call(self.model_name, messages=history, result_format="message", tools=self.tools,
                                       stream=True, incremental_output=incremental_output, **gen_conf)
                    ...
                    if resp.status_code == HTTPStatus.OK:
                        ...
                        if "tool_calls" in assistant_output:
                            ...
                            if tool_call_finish_reason == "tool_calls":
                                ...
                                tool_info["content"] = self.toolcall_session.tool_call(name=tool_name,
                                ...
                                response = Generation.call(self.model_name, messages=history, result_format="message",
                                                           ...
                                                           incremental_output=incremental_output, **gen_conf)
                    else:
                        yield (
                            ans + "\n**ERROR**: model_name=" + self.model_name + "," + resp.output.choices[0].message
    ...
    def _chat_streamly(self, system, history, gen_conf, incremental_output=True):
        ...
        tk_count = 0
        try:
            response = Generation.call(self.model_name, messages=history, result_format="message", stream=True,
                                       incremental_output=incremental_output, **gen_conf)
            for resp in response:
                if resp.status_code == HTTPStatus.OK:
                    ans = resp.output.choices[0]["message"]["content"]
                    ...
                    yield (
                        ans + "\n**ERROR**: model_name=" + self.model_name + "," + resp.message
                        if not re.search(r" (key|quota)", str(resp.message).lower())
                        else "Out of credit. Please set the API key in **settings > Model providers.**"
                    )
        except Exception as e:
            print(traceback.format_exc())
            yield ans + f"\n**  {self.model_name}大模型调用异常! ERROR**: " + str(e)
...
class OllamaChat(Base):
    def __init__(self, key, model_name, base_url=None, **kwargs):
        ...
        self.client = Client(host=kwargs["base_url"]) if not key or key == "x" else Client(host=kwargs["base_url"],
                                                                                           headers={
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        try:
            ...
            yield "**ERROR**: model_name=" + self.model_name + "," + str(e)
            yield 0
...
class BedrockChat(Base):
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        if self.model_name.split(".")[0] == "ai21":
            try:
                response = self.client.converse(modelId=self.model_name, messages=history, inferenceConfig=gen_conf,
                                                system=[{"text": (system if system else "Answer the user's message.")}])
                ans = response["output"]["message"]["content"][0]["text"]
                return ans, num_tokens_from_string(ans)
        ...
        try:
            ...
            streaming_response = self.client.converse_stream(
                modelId=self.model_name, messages=history, inferenceConfig=gen_conf,
                system=[{"text": (system if system else "Answer the user's message.")}]
            )
...
class BaiduYiyanChat(Base):
    ...
    def chat_streamly(self, system, history, gen_conf):
        ...
        try:
            ...
            return ans + "\n**ERROR**: model_name=" + self.model_name + "," + str(e), 0
