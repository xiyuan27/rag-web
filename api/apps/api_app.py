...
def completion():
    """
    基于会话的问答接口，支持流式和非流式响应。主要功能如下：
       1.身份验证：从请求头中获取 Token 并验证其有效性。
       2.获取对话上下文：根据 conversation_id 获取会话对象。
       3.消息预处理：过滤系统消息和无效的 assistant 消息，并生成唯一 message_id。
       4.定义内部函数：
           fillin_conv：更新会话记录中的回复内容与引用信息。
           rename_field：将引用数据中的字段名 docnm_kwd 重命名为 doc_name。
       5.判断会话来源：
           如果是 agent 类型会话，则使用 Canvas 对象执行对话逻辑。
            否则使用 DialogService 进行对话处理。
       6.支持流式（SSE）与非流式响应：
           若为流式，通过 text/event-stream 实时返回结果。
           若为非流式，等待完整回答后统一返回。
       7.异常处理：捕获并返回服务端错误信息
    """
    print("=========基于会话的问答接口")
