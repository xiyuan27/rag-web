...
def list_convsersation():
    ...
    try:
        #去掉对话的权限控制, 暂时让所有人都可以list对话列表
        # userTenants = UserTenantService.query(user_id=current_user.id)
        # if userTenants and len(userTenants) > 0:
        #      userTenant = userTenants[0]
        # if not DialogService.query(tenant_id=userTenant.tenant_id, id=dialog_id):
