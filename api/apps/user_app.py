...
def login():
    ...
    if user:
        ...
        user.save()
        user_tenants = UserTenantService.get_tenants_by_user_id(user_id=user.id)
        if user_tenants and len(user_tenants) > 0:
        ...
        schema:
          ...
          properties:
            ...
            email:
              ...
              description: User email.
    """
    user_tenants = UserTenantService.get_tenants_by_user_id(user_id=current_user.id)
    if user_tenants and len(user_tenants) > 0:
        user_tenant = user_tenants[0]
        role = user_tenant['role']
    if not role:
...
def rollback_user_registration(user_id):
    ...
    try:
        ...
        pass
    try:
        u = UserTenantService.get_tenants_by_user_id(user_id=user_id)
        ...
        pass
    try:
        TenantLLM.delete().where(TenantLLM.tenant_id == u[0].id).execute()
    except Exception:
        pass
